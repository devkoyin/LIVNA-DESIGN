...
document.getElementById('commission-form')?.addEventListener('submit', async (e) => {
  ...
  /* ── Wait 5 seconds then show confirmation popup ─────── */
  setTimeout(() => {
    btn.disabled = false;
    btn.innerHTML = 'Submit Commission <span class="btn-icon" aria-hidden="true"></span>';
...
(function () {
  const textarea  = document.getElementById('design-vision');
  const countEl   = document.getElementById('vision-count');
  const counter   = document.getElementById('vision-counter');
  ...
  textarea.addEventListener('input', () => {
    ...
    const pct  = Math.min((len / MIN) * 100, 100);
